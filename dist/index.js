...
async function pr() {
    const octokit = new dist_node.Octokit({ auth: lib_core.getInput('GITHUB_TOKEN') })
    ...
    if (eventData.action === 'opened') {
        ...
        lib_core.debug(`Unsupported event: ${process.env.GITHUB_EVENT_NAME}`)
        return
    }

    if (!diff) {
        lib_core.debug('No diff found')
        return
    }

    const parsedDiff = parse_diff_default()(diff)

    const excludePatterns = lib_core.getInput('exclude')
        .split(',')
        .map(s => s.trim())

    const includePatterns = lib_core.getInput('include')
        .split(',')
        .map(s => s.trim())
